...
if($user_ok != true || $log_username == ""){
	header("location: http://codersbook.site50.net");
    exit();
}
$notification_list = "";
...
mysqli_query($db_con, "UPDATE users SET notescheck=now() WHERE username='$log_username' LIMIT 1");
?><?php
$friend_requests = "";
...
if($numrows < 1){
	...
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		...
		$thumbquery = mysqli_query($db_con, "SELECT avatar FROM users WHERE username='$user1' LIMIT 1");
...
<style type="text/css">
div#notesBox{float:left; width:430px; border:#07c 1px dashed; margin-right:60px; padding:10px;}
div#friendReqBox{float:left; width:430px; border:#07c 1px dashed; padding:10px;}
