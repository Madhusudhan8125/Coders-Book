...
if (isset($_POST['action']) && $_POST['action'] == "status_post"){
	...
	$friends = array();
	$query = mysqli_query($db_con, "SELECT user1 FROM friends WHERE user2='$log_username' AND accepted='1'");
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) { array_push($friends, $row["user1"]); }
	$query = mysqli_query($db_con, "SELECT user2 FROM friends WHERE user1='$log_username' AND accepted='1'");
